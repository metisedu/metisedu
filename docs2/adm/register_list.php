...
function get_date_total_register($date){
	global $g5;

	$sql = " select count(*) AS cnt
			   from {$g5['member_table']}
			  where SUBSTRING(mb_datetime,1,10) = '$date'
			  ";
			...
			<div class="col-md-12">
				...
				<div class="sch_last">
					...
					<select name="mb_route" id="mb_route" class="frm_input">
						...
						for($i = 0; $row = sql_fetch_array($sql); $i++){
							echo"<option value='".$row['mb_route']."'>".$row['mb_route']."</option>";
						}
						?>
					</select>
				</div>
							...
							datasets: [{
								...
								data: [
									...
									for($date = $bt_day; $date >= 0; $date--){
										...
										$amount = get_date_total_register($a_to_day);
										$ct_amt[] = "'".$amount['cnt']."'";
									}
									echo implode(",",$ct_amt);
									?>
								...
								data: [
									...
									for($date = $bt_day; $date >= 0; $date--){
										...
										$amount2 = get_date_total_register($a_to_day2);
										$ct_amt2[] = "'".$amount2['cnt']."'";
									}
				...
				<div class="tbl_head01 tbl_wrap">
					...
					for($date = 0; $date <= $bt_day; $date++){
						...
						$a_to_day2 = str_replace("/","-",$one_year_ago);
						$amount = get_date_total_register($a_to_day);
						$amount2 = get_date_total_register($a_to_day2); // 1년전
