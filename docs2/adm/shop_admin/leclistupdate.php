...
if ($_POST['act_button'] == "선택수정") {
    ...
    for ($i=0; $i<count($_POST['chk']); $i++) {
		...
		*/
        ...
        $sql = "update han_shop_lec
                   set ca_id2         = '".sql_real_escape_string($p_ca_id2)."',
                       it_update_time = '".G5_TIME_YMDHIS."'
                 where lt_id   = '".preg_replace('/[^a-z0-9_\-]/i', '', $_POST['lt_id'][$k])."' ";

        sql_query($sql);

		//if( function_exists('shop_seo_title_update') ) shop_seo_title_update(preg_replace('/[^a-z0-9_\-]/i', '', $_POST['lt_id'][$k]), true);
    }
    ...
    for ($i=0; $i<count($_POST['chk']); $i++) {
        ...
        $lt_id = preg_replace('/[^a-z0-9_\-]/i', '', $_POST['lt_id'][$k]);
        include ('./lecdelete.inc.php');
    }
}

goto_url("./leclist.php?sca=$sca&amp;sst=$sst&amp;sod=$sod&amp;sfl=$sfl&amp;stx=$stx&amp;page=$page");
?>
