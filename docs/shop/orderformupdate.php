...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
	...
	$od_item_names_arr[] = $row["it_name"];
}
$od_item_names = implode("|", $od_item_names_arr);
...
else if ($od_settle_case == "METISMETAMASK")
{
    $od_receipt_point   = $i_temp_point;
    $od_receipt_price   = 0;
    ...
    if ($tot_ct_price >= 10) {
        ...
        if ($staking) {
            $od_receipt_price = floor($tot_ct_price / 100 * $staking["hs_discount"]);
        }
    }
    $od_misu            = $i_price - $od_receipt_price;
    $od_bank_account    = '';
...
if($od_settle_case == 'KAKAOPAY')
    $od_pg = 'KAKAOPAY';
if($od_settle_case == 'METISMETAMASK')
    $od_pg = 'METISMETAMASK';
...
// 이더 주소 추가
$apiUrl = 'http://15.165.159.201/api/v1/eth/account?order_id='.$od_id;
...
if ($response->code != 200) {
    alert("입금 주소를 생성하던 도중 오류가 발생했습니다. 관리자에게 문의해주세요.");
}
...
if($od_status == '입금'){
	$sQuery = " SELECT *
				...
				WHERE od_id = '".$od_id."'
				";
	$date_check = sql_fetch($sQuery);

	$it = get_item($date_check['it_id']);

	$ct_start_date = date("Y-m-d");
	if($it['it_lect_type'] == '기간'){
		$ct_end_date = date("Y-m-d", strtotime("+".$it['it_lect_date']." day"));
	}else if($it['it_lect_type'] == '일수'){
		$ct_end_date = $it['it_lect_day'];
	}else{
		$ct_end_date = "2100-12-31";
	}

	sql_query("UPDATE {$g5['g5_shop_cart_table']} SET ct_start_date = '".date("Y-m-d")."', ct_end_date = '".$ct_end_date."' WHERE od_id = '".$od_id."' ");
	sql_query("UPDATE {$g5['g5_shop_order_table']} SET od_start_date = '".date("Y-m-d")."', od_end_date = '".$ct_end_date."' WHERE od_id = '".$od_id."' ");
}
