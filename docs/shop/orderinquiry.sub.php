...
.class-filter .period_date button.btn_date {
    ...
    border-bottom: 1px solid #edad58;
    margin-left: 5px;
	border:0px;
}
</style>
...
<div class="col-md-9 col-sm-12 col-xs-12 my_con">
	<div class="mypage-class">
		<div class="mypage-class-menu">
			<ul>
				<li><a href="/shop/orderinquiry.php" class="active"><!-- 결제내역 -->Payment list</a></li>
				<li><a href="/shop/refund.php" class=""><!-- 환불내역 -->Refund</a></li>
			</ul>
		</div>

        <div>연동된 메타마스크 계정: <span id="eth-address"><?php echo strlen($member["mb_address"]) > 10 ? $member["mb_address"] : "없음" ?></span> <button onclick="updateAddress().then(console.log);">재연동</button></div>
        <script>
		...
		<div class="mypage-payment-list no-after">
			<div class="class-filter">
				...
				<div class="col-md-6">
					<div class="period">
						<ul>
							<li><a href="javascript:set_date(3);" class="<?php if($bt_mon == 3) echo"active"; ?>"><!-- 3개월 -->3 months</a></li>
							<li><a href="javascript:set_date(6);" class="<?php if($bt_mon == 6) echo"active"; ?>"><!-- 6개월 -->6 months</a></li>
							<li><a href="javascript:set_date(12);" class="<?php if($bt_mon == 0) echo"active"; ?>"><!-- 12개월 -->12 months</a></li>
						</ul>
					</div>
				</div>
				<div class="col-md-6">
					...
					<form name="nForm" id="nForm" method="post">
						<input type="text" name="s_date" id="s_date" class="form_control input_period" value="<?php echo $s_date; ?>" readonly> - <input type="text" name="e_date" id="e_date" class="form_control input_period"  value="<?php echo $e_date; ?>" readonly>
						<button type="submit" class="btn_date"><!-- 조회 -->Search</button>
					</form>
					</div>
				</div>
				...
				for ($i=0; $row=sql_fetch_array($result); $i++)
				{
					...
					//-- 고객요청으로 입금확인중~ 문구노출 가리기 YHJ 2021.03.12 아래구문 추가
					$od_status = '';
					//-- 끝
					...
					<div class="row">
						...
						<div class="col-md-3">
							...
							<div class="payment_price">
								<dl>
									<dt><!-- 판매금액 -->Price</dt>
									<dd><?php echo display_price($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']); ?></dd>
									<dt><!-- 할인금액 -->Discount</dt>
									<dd>-<?php echo number_format($row['od_cart_coupon'] + $row['od_coupon'])?>MTS</dd>
								</dl>
								<dl class="last">
									<dt><!-- 최종 결제금액 -->Total Price</dt>
									<dd><?php echo display_price($row['od_cart_price'] - ( $row['od_cart_coupon'] + $row['od_coupon'] )); ?> </dd>
								</dl>
							</div>
						</div>
						<div class="col-md-5">
							<div class="payment_price refund">
								<dl>
									<dt><!-- 결제수단 -->Payment <!-- method --></dt>
									<dd><?php echo $row['od_settle_case']; ?></dd>
									<?php if($row['od_status'] == '입금' || $row['od_status'] == '완료'){ ?>
									<dt>결제일시</dt>
									<dd><?php echo date("Y.m.d H:i", strtotime($row['od_receipt_time']))?></dd>
									<?php }else{ ?>
									<dt><!-- 신청일시 -->Date</dt>
									<dd><?php echo date("Y.m.d H:i", strtotime($row['od_time']))?></dd>
									<?php } ?>
								</dl>
							</div>
							<style>
							...
							<div class="modal fade" id="refundModal" tabindex="-1" role="dialog" aria-labelledby="refundModalLabel" aria-hidden="true">
							  <div class="modal-dialog" role="document">
								<div class="modal-content">
								  ...
								  <br>
									Hello! This is <?php echo $config['cf_title']; ?>.<br><br>

									환불 신청을 원하시면 <a href="mailto:<?php echo $default['de_admin_info_email']; ?>"><?php echo $default['de_admin_info_email']; ?></a> 으로<br>
									메일 주시면 신속히 처리해 드리겠습니다.<br><br>
									<a href="mailto:<?php echo $default['de_admin_info_email']; ?>" class="refund_email">이메일 환불 신청</a>
								  </div>
								  <!--
				...
				if ($i == 0)
					echo '<li class="empty_table">You do not have the order.</li>';
				?>
			</div>
