			...
			<div class="col-md-9 col-sm-12 col-xs-12 my_con">
				<div class="mypage-class">
					<div class="mypage-class-menu">
						<ul>
							<li><a href="/shop/mypage.php#class"><!-- 수강중인 강의 -->Course</a></li>
							<li><a href="/shop/cart.php" class="active"><!-- 장바구니 -->Cart</a></li>
							<li><a href="/shop/mypage.php#done"><!-- 수강 완료 강의 -->Finished course</a></li>
						</ul>
					</div>
							...
							<tbody>
								...
								for ($i=0; $row=sql_fetch_array($result); $i++)
								{
									...
									$sQuery = " SELECT a.mb_id
												...
												AND   b.it_id = '".$row['it_id']."'
												ORDER BY ir_no ASC
												LIMIT 1
												";
									$rst = sql_fetch($sQuery);
									$tc = get_member($rst['mb_id']);
									$row['it_t_name'] = $tc['mb_name'];
								...
								<tr class="item-list">
									...
									<div class="media">
										...
										<div class="item-list-name">
											<a href="<?php echo shop_item_url($row['it_id']); ?>"><?php echo $it_name; ?></a>
											<span class="author"><?php echo $row['it_t_name']; ?> | <?php echo $ca_id_slt[$row['ca_id']]; ?></span>
											<!--<span class="date">구매일 2020.01.01</span>-->
										</div>
									</div>
									</td>
									<td class="w35 text-center period">
										<?php echo get_lec_date($row['it_id'], $row['ct_end_date']); ?>
									</td>
									<td class="w15 text-right btn_wrap">
										<input type="checkbox" name="ct_chk[<?php echo $i; ?>]" value="1" id="ct_chk_<?php echo $i; ?>" class="selec_chk">
										<input type="hidden" name="it_id[<?php echo $i; ?>]" value="<?php echo $row['it_id']; ?>">
										<input type="hidden" name="it_name[<?php echo $i; ?>]" value="<?php echo get_text($row['it_name']); ?>">
										<a href="javascript:void(0);" class="text-center payment_btn"><!-- 결제하기 -->Payment</a>
										<a href="javascript:void(0);" class="text-center delete_btn"><!-- 삭제 -->Delete</a>
									</td>
								</tr>
								<?php
									$tot_point      += $point;
								...
								if ($i == 0) {
									echo '<tr><td colspan="7" class="empty_table"><!-- 장바구니에 담긴 상품이 없습니다. -->There are no products in the cart.</td></tr>';
								} else {
									// 배송비 계산
									$send_cost = get_sendcost($s_cart_id, 0);
									echo'<input type="hidden" name="url" value="./orderform.php">';
...
<div id="sod_bsk" class="od_prd_list" style="display:none;">

    <form name="frmcartlist11" id="sod_bsk_list11" class="2017_renewal_itemform" method="post" action="<?php echo $cart_action_url; ?>">
