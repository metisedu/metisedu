...
#settle_bank, #settle_metis_metamask {
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12 add_bottom_60">
				<span class="sub-top-title"><!-- 강의 결제하기 -->Payment</span>
			</div>
		</div>
		<div class="row">
			<div class="col-md-9">
				<div class="class_info add_bottom_60">
					...
					<div class="media">
						...
						<div class="item-list-name">
							...
							<?php echo $cp_button; ?>
							<span class="author"><?php echo get_lec_date($row['it_id'], $row['ct_end_date']); ?>, <?php echo $row['it_t_name']; ?> | <?php echo $ca_id_slt[$row['ca_id']]; ?></span>
						</div>
					</div>
					<?php
						$tot_point      += $point;
				...
				<div class="coupon_info add_bottom_60">
					<div class="payment_sub_tit">
						<!-- 쿠폰정보 -->Coupon Infomation
					</div>
					<div class="media" style="list-style:none;">
						<?php
						// 쿠폰정보
						$sql = " select *
									from {$g5['g5_shop_coupon_table']}
						...
						// 랜덤쿠폰
						$sql = " SELECT *
								 FROM han_shop_coupon_log AS a
								 INNER JOIN han_shop_coupon AS b ON a.cp_id = b.cp_id
								 WHERE a.mb_id IN ( '{$member['mb_id']}' )
								 AND   b.cp_start <= '".G5_TIME_YMD."'
								 AND   b.cp_end >= '".G5_TIME_YMD."'
								 AND   b.cp_method2 = '2'
								 AND   a.rand_id != ''
								 AND   b.cp_minimum <= '$tot_sell_price'
								 ";
						$rand = sql_query($sql);
						...
						if($count > 0) {
							for($i=0; $row=sql_fetch_array($result); $i++) {
								...
								echo'<li>';
								echo'<input type="hidden" name="o_cp_id[]" value="'.$row['cp_id'].'">';
								echo'<input type="hidden" name="o_cp_prc[]" value="'.$dc.'">';
								echo'<input type="hidden" name="o_cp_subj[]" value="'.$row['cp_subject'].'">';
								echo'<span class="coupon_text">'.get_text($row['cp_subject']).' <span class="coupon_price">'.number_format($dc).'MTS</span> <button type="button" class="od_cp_apply coupon_use">쿠폰사용하기</button></span>';
								echo'</li>';
							}
							...
							for($i=0; $row=sql_fetch_array($rand); $i++) {
								...
								echo'<li>';
								echo'<input type="hidden" name="o_cp_id[]" value="'.$row['rand_id'].'">';
								echo'<input type="hidden" name="o_cp_prc[]" value="'.$dc.'">';
								echo'<input type="hidden" name="o_cp_subj[]" value="'.$row['cp_subject'].'">';
								echo'<span class="coupon_text">'.get_text($row['cp_subject']).' <span class="coupon_price">'.number_format($dc).'MTS</span> <button type="button" class="od_cp_apply coupon_use">쿠폰사용하기</button></span>';
								echo'</li>';
							...
							echo '<span class="coupon_text"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> <!-- 보유쿠폰이 없습니다 -->no coupons</span>';
						}
						?>
						<!-- } 쿠폰 선택 끝 -->
						...
						<li>
							...
							<div class="sod_ifop_t">
								<strong id="sc_cp_price">0</strong>MTS
							</div>
						</li>
						<?php } ?>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="media payment_input">
								<label for="od_name"><!-- 이름 -->Name</label>
								<input type="text" id="od_b_name" name="od_b_name" value="<?php echo $member['mb_name']; ?>" required class="required" placeholder="To type a name, please.">
								<label for="od_email"><!-- 이메일 -->E-mail</label>
								<input type="text" id="od_email" name="od_email" value="<?php echo $member['mb_email']; ?>" required class="required" placeholder="Enter an email.">
								<label for="od_email"><!-- 연락처 -->H.P</label>
								<input type="text" id="od_b_tel" name="od_b_tel" value="<?php echo $member['mb_hp']; ?>" required class="required" placeholder="Enter the contact." onKeyup="this.value=this.value.replace(/[^0-9]/g,'');">
							</div>
						</div>
						<div class="col-md-6">
							<div class="media payment_method">
								<label><!-- 결제방식 -->Payment method</label>
                                ...
                                if(true) {
                                    $multi_settle++;
                                    echo '<input type="radio" id="od_settle_metis_metamask" name="od_settle_case" value="METISMETAMASK" '.$checked.'> <label for="od_settle_metis_metamask" class="lb_icon bank_iconx">MTS</label>'.PHP_EOL;
                                    $checked = '';
                                }

								// 무통장입금 사용
								if (false && $default['de_bank_use']) {
									$multi_settle++;
								...
								if ($default['de_bank_use']) {
									...
									echo '<label for="od_bank_account" class="sound_only" style="display:none !important;">입금할 계좌</label>';
									echo $bank_account;
								...
								<label><!-- 결제 약관 동의 -->Pay Agreement Consent</label>
								<div class="agree_box">
									<ul>
										<li><a href="javascript:void(0);" onclick="$(this).parent('li').children('div.agree_text').toggle();">1. <!-- 온라인 이용약관 -->User Agreement <span class="">View <i class="fa fa-angle-down" aria-hidden="true"></i></a></span>
											<div class="agree_text">
											<?php
											include_once("./agree_text1.php");
											?>
											</div>
										</li>
										<li><a href="javascript:void(0);" onclick="$(this).parent('li').children('div.agree_text').toggle();">2. <!-- 개인정보취급방침 -->Privacy Policy <span class="">View <i class="fa fa-angle-down" aria-hidden="true"></i></a></span>
											<div class="agree_text">
											<?php
											include_once("./agree_text2.php");
											?>
											</div>
										</li>
									</ul>
								...
								<div class="agree_check">
									...
									<label class="toggle">
									  ...
									  <span class="toggle__label">
										<span class="toggle__text"><!-- 강의명, 가격 등을 확인 하였으며 이용약관에 모두 동의 합니다. -->I checked the lecture, price, etc. and I agree to the terms and conditions.</span>
									  </span>
									</label>
									</div>
									<div class="require_tax">* <!-- 세금계산서 요청은 -->Request a tax invoice at <?php echo $config['cf_admin_email']; ?>.</div>
								</div>
							</div>
						</div>
					</div>
			...
			<aside class="col-md-3">
				<div class="box_style_4 sub_detail_action payment_aside">
					...
					<div class="payment_price">
						<dl>
							<dt><!-- 수강료 -->Course Payment</dt>
							<dd><?php echo number_format($tot_sell_price); ?>MTS</dd>
						</dl>
					</div>
					...
					<div class="payment_discount">
						...
						<dl>
							<!--<dt>강의할인</dt>
							<dd>-<?php echo number_format(100000)?>MTS</dd>-->
							<dt><!-- 쿠폰할인 -->Coupon discount</dt>
							<dd>-<span id="od_cp_price">
                                    <?php
                                    $dis_price = 0;
                                    if ($tot_price >= 10) {
                                        $sql = " select *
                                            ...
                                            where mb_id = '{$member['mb_id']}'
                                              and hs_startdate <= '".date('Y-m-d')."'
                                              and hs_enddate >= '".date('Y-m-d')."' ";
                                        $staking = sql_fetch($sql);

                                        if ($staking) {
                                            $dis_price = floor($tot_price / 100 * $staking["hs_discount"]);
                                        }
                                    }
                                    echo $dis_price;
                                    ?>
                                </span>MTS</dd><!--ct_tot_coupon-->
						</dl>
					...
					<div class="sub_detail_action_btn">
						...
						<strong id="ct_tot_price" style="display:none;"><?php echo number_format($tot_price - $dis_price); ?> MTS</strong>
						<!-- 총결제금액 -->Total Price
						<span class="print_price"><?php echo number_format($tot_price - $dis_price); ?></span>MTS
						</div>
					</div>
...
$(function() {
    ...
    $(document).on("click", ".cp_cancel", function() {
		...
		$(this).parent("span").children(".od_cp_apply").show();
        $(this).remove();
    });
    ...
    $("#od_coupon_btn").click(function() {
        ...
        if(price <= 0) {
            alert('상품금액이 0MTS이므로 쿠폰을 사용할 수 없습니다.');
            return false;
        }
        $.post(
            "./ordercoupon.php",
    ...
    $(document).on("click", ".od_cp_apply", function() {
        ...
        if(price == 0) {
            if(!confirm(subj+"쿠폰의 할인 금액은 "+price+"MTS입니다.\n쿠폰을 적용하시겠습니까?")) {
                return false;
            }
        }
        ...
        if(!$(".od_coupon_cancel").size()){
            $(this).hide();
			$(this).after("<button type=\"btton\" class=\"cp_cancel\">취소</button>");
		}
    });
    ...
    $(document).on("click", ".od_coupon_cancel", function() {
        ...
        $(this).find(".od_cp_apply").show();
        $(this).remove();
        //$("#sc_coupon_btn").text("쿠폰사용하기");
        //$("#sc_coupon_cancel").remove();
    });
...
function calculate_order_price()
{
	...
	$(".payment_tot_price .print_price").text(number_format(String(tot_price)));
    <?php if($temp_point > 0 && $is_member) { ?>
    calculate_temp_point();
    <?php } ?>
}
...
{
    //alert(<?php //echo json_encode($_SERVER['HTTP_USER_AGENT']); ?>//)
    //return false;

    //alert(JSON.stringify(<?php //echo json_encode($_GET); ?>//));
    if (localStorage.getItem("FORCEAGENT") === "app") {
        alert("앱에서는 결제가 불가능합니다. 홈페이지나 모바일웹을 통해 결제 부탁드립니다.");
        return false;
    }
	...
	*/
    ...
    if(f.od_email.value=='' || f.od_email.value.search(/(\S+)@(\S+)\.(\S+)/) == -1)
        error_field(f.od_email, "Enter an email.");
    ...
    check_field(f.od_b_name, "To type a name, please.");
    check_field(f.od_b_tel, "Enter the contact.");
	...
	*/
    ...
    if( jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
        ...
        f.buyertel.value    = f.od_hp.value ? f.od_hp.value : f.od_b_tel.value;
        f.recvname.value    = f.od_b_name.value;
        f.recvtel.value     = f.od_b_tel.value;
