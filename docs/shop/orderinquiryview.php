...
<div id="sod_fin">
    ...
    <section id="sod_fin_list">
        ...
        <div class="tbl_head03 tbl_wrap" style="overflow:auto;">
			<table>
	            <thead>
	            <tr class="th_line">
	            	<th scope="col" id="th_itname"><!-- 상품명 -->Product Name</th>
	                <th scope="col" id="th_itqty"><!-- 총수량 -->Total Number</th>
	                <th scope="col" id="th_itprice"><!-- 판매가 -->Price</th>
	                <th scope="col" id="th_itsum"><!-- 소계 -->Subtotal</th>
	                <th scope="col" id="th_itst"><!-- 상태 -->Status</th>
	            </tr>
	            </thead>
	            <tbody>
	            <?php
    ...
    <div class="sod_left">
        ...
        <section id="sod_fin_orderer" style="display:none;">
            <h3>주문하신 분</h3>
            ...
            <div class="tbl_head01 tbl_wrap">
                ...
                <tr>
                    <th scope="row"><!-- 핸드폰 -->H.P</th>
                    <td><?php echo get_text($od['od_hp']); ?></td>
                </tr>
                <tr>
                    <th scope="row"><!-- 주 소 -->Adress</th>
        ...
        <section id="sod_fin_receiver">
            <h3><!-- 결제자 정보 -->Payer Information</h3>
        ...
        <section id="sod_fin_dvr" style="display:none;">
            <h3>배송정보</h3>
    ...
    <div class="sod_right">
        <ul id="sod_bsk_tot2">
            <li class="sod_bsk_dvr">
                <span><!-- 주문총액 -->Total Order</span>
                <strong><?php echo number_format($od['od_cart_price']); ?> MTS</strong>
            </li>
            <?php if($od['od_cart_coupon'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span>개별상품 쿠폰할인</span>
                <strong><?php echo number_format($od['od_cart_coupon']); ?> MTS</strong>
            </li>
            <?php } ?>
            <?php if($od['od_coupon'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span>주문금액 쿠폰할인</span>
                <strong><?php echo number_format($od['od_coupon']); ?> MTS</strong>
            </li>
            <?php } ?>
            <?php if ($od['od_send_cost'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span>배송비</span>
                <strong><?php echo number_format($od['od_send_cost']); ?> MTS</strong>
            </li>
            <?php } ?>
            <?php if($od['od_send_coupon'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span>배송비 쿠폰할인</span>
                <strong><?php echo number_format($od['od_send_coupon']); ?> MTS</strong>
            </li>
            <?php } ?>
            <?php if ($od['od_send_cost2'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span>추가배송비</span>
                <strong><?php echo number_format($od['od_send_cost2']); ?> MTS</strong>
            </li>
            <?php } ?>
            <?php if ($od['od_cancel_price'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span>취소금액</span>
                <strong><?php echo number_format($od['od_cancel_price']); ?> MTS</strong>
            </li>
            <?php } ?>
            <li class="sod_bsk_cnt">
                <span><!-- 총계 -->Toal Amount</span>
                <strong><?php echo number_format($tot_price); ?> MTS</strong>
            </li>

            <li class="sod_fin_tot"><span><!-- 총 구매액 -->Total Purchases</span><strong><?php echo display_price($tot_price); ?></strong></li>
            <?php
            if ($misu_price > 0) {
            echo '<li class="sod_fin_tot">';
            echo '<span>Unselled Amount</span>'.PHP_EOL;
            echo '<strong>'.display_price($misu_price).'</strong>';
            echo '</li>';
            }
            ?>
            <li id="alrdy" class="sod_fin_tot">
            	<span><!-- 결제액 -->Payment Amount</span>
                ...
                <div>
                    ...
                    <p><span class="title">실결제</span><?php echo number_format($od['od_receipt_price']); ?>MTS</p>
                </div>
                <?php } ?>
            </li>
        </ul>

        <section id="sod_fin_metamask" <?php echo $misu_price === 0 ? 'style="display: none;"' : false; ?>>
            <button style="border: none; padding:0; margin: 0; width: 100%;" onclick="requestPayment()">
                <img src="/img/btn_metamask.png" style="width:100%;"/>
            </button>
        </section>
        ...
        <script>
            async function requestPayment() {
                ...
                if (window.ethereum) {
                    ...
                    try {
                        alert("결제가 처리되는 데에는 약 3분~5분 정도가 소요되니 중복으로 결제되지 않도록 주의해주세요!");
                        ...
                        var targetWallet = "<?php echo $od["od_deposit_address"]; ?>";
                        var tokenContractAddr = "0xA9598333B99d14D90Bc81CAd8Af82C4C70625e75";
                        // var tokenContractAddr = "0xfab46e002bbf0b4509813474841e0716e6730136";
                        var tokenContract = new window.web3.eth.Contract(token_abi, tokenContractAddr);
                        var decimals = window.web3.utils.toBN(10).pow(window.web3.utils.toBN(18));
                        var amount = window.web3.utils.toBN(<?php echo $misu_price; ?>);
                        await tokenContract.methods.transfer(targetWallet, amount.mul(decimals)).send({
                           from: accounts[0]
                        });
        ...
        <section id="sod_fin_pay">
            <h3><!-- 결제정보 -->Payment Information</h3>
            <ul>
	            <li>
	                <strong><!-- 주문번호 -->Order number</strong>
	                <span><?php echo $od_id; ?></span>
	            </li>
	            <li>
	                <strong><!-- 주문일시 -->Data and Time</strong>
	                <span><?php echo $od['od_time']; ?></span>
	            </li>
	            <li>
	                <strong><!-- 결제방식 -->Payment Method</strong>
	                <span><?php echo ($easy_pay_name ? $easy_pay_name.'('.$od['od_settle_case'].')' : check_pay_name_replace($od['od_settle_case']) ); ?></span>
	            </li>
	            <li>
	                <strong><!-- 결제금액 -->Payment Amount</strong>
	            ...
	            ?>
	            <li>
	                <strong><!-- 결제일시 -->Payment Date</strong>
        ...
        <section id="sod_fin_cancel">
            ...
            if ($cancel_price == 0) {
                if ($custom_cancel) {
            ?>
            <button type="button" class="sod_fin_c_btn"><!-- 주문 취소하기 -->Cancel Order</button>
			<div id="sod_cancel_pop">
	            <div id="sod_fin_cancelfrm">
	            	<h2><!-- 주문취소 -->Cancel Order</h2>
	                ...
	                <input type="text" name="cancel_memo" id="cancel_memo" required class="frm_input required" size="40" maxlength="100" style="width:100%;margin-left:0px;" placeholder="취소사유">
