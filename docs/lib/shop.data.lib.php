...
// 강의
function get_shop_lec($lt_id, $is_cache=false, $add_query=''){

    global $g5, $g5_object;

    $add_query_key = $add_query ? 'shop_'.md5($add_query) : '';

    $item = $is_cache ? $g5_object->get('shop', $lt_id, $add_query_key) : null;

    if( !$item ){
        $sql = " select * from han_shop_lec where lt_id = '{$lt_id}' $add_query ";
        $item = sql_fetch($sql);

        $g5_object->set('shop', $lt_id, $item, $add_query_key);
    }
