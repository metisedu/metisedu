...
function shop_seo_title_update($it_id, $is_edit=false){
	...
	for($i = 0; $row = sql_fetch_array($sql); $i++){
		if($row['it_id'] == $item['it_id']){
			$no = $row['rNum'];
		}else{
			$it_seo_title = exist_seo_title_recursive('shop', generate_seo_title($item['it_name']), $g5['g5_shop_item_table'], $row['it_id']);
			//$it_seo_title = $it_seo_title."-".$row['rNum'];

			$sql = " update `{$g5['g5_shop_item_table']}` set it_seo_title = '{$it_seo_title}' where it_id = '{$row['it_id']}' ";
			sql_query($sql);
		}
	}
	*/
	// 같은 상품명이 있을 경우 구분 end
