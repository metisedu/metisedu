...
// 한페이지에 보여줄 행, 현재페이지, 총페이지수, URL, AJAX
function get_paging2($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<a href="javascript:'.str_replace("#page","1",$url).'" class="pg_page pg_start">처음</a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="javascript:'.str_replace("#page",($start_page-1),$url).'" class="pg_page pg_prev">이전</a>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="javascript:'.str_replace("#page",$k,$url).'" class="pg_page">'.$k.'<span class="sound_only">페이지</span></a>'.PHP_EOL;
            else
                $str .= '<span class="sound_only">열린</span><strong class="pg_current">'.$k.'</strong><span class="sound_only">페이지</span>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<a href="javascript:'.str_replace("#page",($end_page+1),$url).'" class="pg_page pg_next">다음</a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="javascript:'.str_replace("#page",$total_page,$url).'" class="pg_page pg_end">맨끝</a>'.PHP_EOL;
    }
...
// 포인트 부여, 사이트 포인트 사용여부 때문에, 기존 함수 망치지 않고 2 생성, 수정은 포인트사용여부만 주석처리 LHH 2021-02-25
function insert_point2($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $po_rel_cart_id, $od_id, $it_id, $order_no=0, $po_per=0)
{
    global $config;
    global $g5;
    global $is_admin;
...
// 포인트 부여 코인용 LHH 2021-02-25
function insert_point_metis_coin($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    global $config;
    global $g5;
    global $is_admin;
	...
	$point_metis_sql = " select sum(po_point) as sum_po_point_metis
                from han_point_metis
                where mb_id = '$mb_id' ";
    $point_metis_row = sql_fetch($point_metis_sql);
	$mb_point = $point_metis_row["sum_po_point_metis"];
	
    // 이미 등록된 내역이라면 건너뜀
    if ($rel_table || $rel_id || $rel_action)
    {
        $sql = " select count(*) as cnt from han_point_metis
                  where mb_id = '$mb_id'
                    and po_rel_table = '$rel_table'
                    and po_rel_id = '$rel_id'
                    and po_rel_action = '$rel_action' ";
    ...
    $sql = " insert into han_point_metis
                set mb_id = '$mb_id',
                    po_datetime = '".G5_TIME_YMDHIS."',
                    po_content = '".addslashes($content)."',
                    po_point = '$point',
    ...
    // 포인트를 사용한 경우 포인트 내역에 사용금액 기록
    if($point < 0) {
        insert_use_point_metis($mb_id, $point);
    }

    // 포인트 UPDATE
    $sql = " update {$g5['member_table']} set mb_point_metis = '$po_mb_point' where mb_id = '$mb_id' ";
    sql_query($sql);
...
// 사용포인트 입력
function insert_use_point_metis($mb_id, $point, $po_id='')
{
    global $g5, $config;
...
// 상품분류(대분류)
function option_ca_id_slt($ca_id=""){
	global $ca_id_slt;

	for($i = 1; $i <= count($ca_id_slt); $i++ ){
		$val = ($i * 10);
		$slt = "";
		if(trim($ca_id) == trim($val)){
			$slt = "selected";
		}
		echo"<option value='".trim($val)."' ".$slt.">".$ca_id_slt[$val]."</option>";
	}
}

// 강의 기간
function get_lec_date($it_id, $ct_end_date, $type=""){
	global $g5;
	...
	if($it['it_lect_type'] == "평생"){
		$lec_date = "평생수강";
	}else if($type == 'mypage'){
		$lec_date = $ct_end_date." 까지";
	}else if($it['it_lect_type'] == "기간"){
		$lec_date = $it['it_lect_date']." 일";
	}else{
		$lec_date = $ct_end_date." 까지";
	}
