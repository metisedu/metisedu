...
class item_list
{
    ...
    function run() {
        ...
        $file = $this->list_skin;
		if($file == $_SERVER['DOCUMENT_ROOT']."/skin/shop/basic/")
			$file .= "list.10.skin.php";
...
// 영상 썸네일 생성
function get_mv_thumbnail($img, $width, $height=0, $id='', $is_crop=false)
{
    $str = '';

    $file = G5_DATA_PATH.'/movie/'.$img;
    if(is_file($file))
        $size = @getimagesize($file);
...
function display_price($price, $tel_inq=false)
{
    if ($tel_inq)
        ...
        $price = number_format($price, 0).' MTS';
...
function it_img_upload($srcfile, $filename, $dir)
{
	...
	*/
    ...
    if(strpos($dir, 'movie') !== false) {
		$file = str_replace(G5_DATA_PATH.'/movie/', '', $dir.'/'.$filename);
	}else if(strpos($dir, 'review') !== false) {
		$file = str_replace(G5_DATA_PATH.'/review/', '', $dir.'/'.$filename);
	}else{
...
function get_item_options($it_id, $subject, $is_div='', $is_first_option_title='')
{
    ...
    if($subj_count > 1) {
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            if($row['io_price'] >= 0)
                $price = '&nbsp;&nbsp;+ '.number_format($row['io_price']).' MTS';
            else
                $price = '&nbsp;&nbsp; '.number_format($row['io_price']).' MTS';
...
function display_banner($position, $skin='')
{
    ...
    if(file_exists($skin_path)) {
		...
		$sql = " select * from {$g5['g5_shop_banner_table']} where 1 $sql_device $sql_use and bn_position = '$position' order by bn_order, bn_id desc ";
        $result = sql_query($sql);
...
function get_item($it_id){
	...
	if(!$rst['it_t_name'] && $rst['mb_id']){
		$tc = get_member($rst['mb_id']);

		$rst['it_t_name'] = $tc['mb_name'];
	}else{
		/*
		$sQuery = " SELECT a.mb_id
					FROM han_shop_lec a, han_shop_item_list b
					WHERE (a.lt_id = b.it_id2)
					AND   b.it_id = '".$it_id."'
					ORDER BY ir_no ASC
					LIMIT 1
					";
...
// 카테고리 정보 가져오기
function get_cate($ca_id){
	$sQuery = " SELECT *
				FROM han_shop_category
				WHERE ca_id = '".$ca_id."'
				";
	$rst = sql_fetch($sQuery);
...
// 회원 프로필 사진 가져오기
function get_profile($mb_id, $width='35'){
	$my_img_path = G5_DATA_PATH.'/member_image/'.substr($mb_id,0,2).'/'.$mb_id.'.gif';
	$my_img_url = '';
	if (file_exists($my_img_path)) {
		$my_img_url  = '<img src="'.G5_DATA_URL.'/member_image/'.substr($mb_id,0,2).'/'.$mb_id.'.gif" style="width:'.$width.'px;height:auto;border-radius:'.($width  / 2).';">';
	}
...
function get_use_mv_check($it_id, $mb_id){
	$sQuery = " SELECT *
				FROM han_member_movie
				WHERE it_id = '".$it_id."'
				AND   mb_id = '".$mb_id."'
				ORDER BY mm_time DESC
				";
	//echo"<p>".$sQuery;
	$rst = sql_fetch($sQuery);
	if($rst){
		$sQuery = " SELECT *
					FROM han_shop_movie
					WHERE mv_id = '".$rst['mv_id']."'
					AND   mv_use = '1'
					";
	}else{
		$sQuery = " SELECT *
					FROM han_shop_chapter a, han_shop_movie b, han_shop_item_list c
					WHERE ( a.cp_id = b.cp_id ) AND ( a.lt_id = b.lt_id ) AND ( a.lt_id = c.it_id2 )
					AND   c.it_id = '".$it_id."'
					AND   b.mv_use = '1'
					AND   b.mv_url != ''
					ORDER BY a.cp_order ASC, b.mv_order ASC, mv_id ASC
					LIMIT 1
					";
	}
	//echo"<p>".$sQuery;
	$first_mv = sql_fetch($sQuery);

	return $first_mv;
}

// 회원 강의 로그 쌓기
function insert_mv_log($it_id, $cp_id, $mv_id, $mb_id, $mm_use_time=0){
	$mm_time = date("Y-m-d H:i:s");
	...
	if($mb_id){ // 회원만
		...
		if($rst){
			$sQuery = " UPDATE han_member_movie
							SET		mm_use_time	= '".$mm_use_time."'
							,		mm_time		= '".$mm_time."'
						WHERE	it_id		= '".$it_id."'
						AND		cp_id		= '".$cp_id."'
						AND		mv_id		= '".$mv_id."'
			...
			$sQuery = " INSERT INTO han_member_movie
							SET		it_id		= '".$it_id."'
							,		cp_id		= '".$cp_id."'
							,		mv_id		= '".$mv_id."'
							,		mb_id		= '".$mb_id."'
							,		mm_use_time	= '".$mm_use_time."'
							,		mm_time		= '".$mm_time."'
							";
			sql_query($sQuery);
		}
...
// 강의 진행상황 가져오기
function get_mv_status($mv_id, $mb_id){
	$sQuery = " SELECT *
				FROM han_member_movie
				WHERE mv_id = '".$mv_id."'
				AND   mb_id = '".$mb_id."'
	...
	return ($rst)? "on class":"before class";
}

// 별점
function get_rating($star_score){
	$t_star_score = (5 - ceil($star_score));
	for($i = 0; $i < floor($star_score); $i++){
	...
	if(strpos($star_score, '.0') === false && strpos($star_score, '.')){
