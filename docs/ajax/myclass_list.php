		...
		<ul class="dep2 <?php echo $ac?> mv_list">
			<?php
				for($j = 0; $mv = sql_fetch_array($sql_b); $j++){
					...
					if($mv_status == "on class") $icon = "ing";
					else if($mv_status == "Course completed") $icon = "done";
					else $icon = "";
			...
			<?php
				...
				if($j == 0){
					echo"<li class='mv_url' data-url=''><b style='padding: 10px;display: block;'>The lecture will be open soon.</b></li>";
				}
			?>
		</ul>
	</li>
<?php
	...
	if($i == 0){
		echo"<li style='padding:10px;'>There are no chapters registered.</li></ul>";
	}
}
?>
</ul>
