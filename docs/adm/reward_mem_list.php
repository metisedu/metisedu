...
$qstr = "token=".$token."&amp;stx=".$stx."&amp;sfl=".$sfl;
$query_string = $qstr ? '?'.$qstr : '';



$sql_common = " from han_member ";
...
//$sql_search .= " AND (mb_point > 0 OR mb_point_metis > 0) ";
...
<form name="frm" id="frm" action="./pay_request_update.php" onsubmit="return frm_submit(this);" method="post">
	...
	<div class="tbl_head01 tbl_wrap">
		...
		<tr>
			...
			<th scope="col"><?php echo subject_sort_link('mb_point') ?>미지급합계</th>
			<th scope="col"><?php echo subject_sort_link('mb_point_metis') ?>지급합계</th>
			<th scope="col">보기</th>
		</tr>
		</thead>
		<tbody>
		...
		<tr class="<?php echo $bg; ?>">
			<td class="td_chk">
				...
				<label for="chk_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['mb_no']) ?></label>
				<input type="checkbox" name="chk[]" value="<?php echo $i ?>" id="chk_<?php echo $i ?>">
			</td>
			<td class="td_numsmall"><?php echo $total_count - ($i + ( ( $page-1 ) * $rows ) )?></td>
			<td class=""><?php echo $row['mb_id']?></td>

			<td class=""><?php echo number_format($row['mb_point'])?></td>
			<td class=""><?php echo number_format($row['mb_point_metis'])?></td>

			<td>
				<a href="./reward_mem_form.php?<?php echo $qstr;?>&amp;w=u&amp;mb_no=<?php echo $row['mb_no'];?>" class="btn btn_03">상세보기</a>
			</td>
