	...
	<div class="card-block">
		<div class="row">
			<div class="col-md-12">
				...
				<form name="frmorderlist" class="local_sch01 local_sch03 local_sch">
				...
				<select name="sel_field" id="sel_field">
					...
					<option value="od_item_names" <?php echo get_selected($sel_field, 'od_item_names'); ?>>강의명</option>
				</select>
				...
				<form name="search_frm" class="local_sch03 local_sch"> -->
				<div>
					<strong>주문상태</strong>
				...
				<div class="sch_last">
					...
					<button type="button" id="btn_excel" class="btn btn_03" style="background-color:#398239;">엑셀다운</button>
					<script>
					$(document).ready(function(){
						$("#btn_excel").on("click", function(){
							$("form[name=frmorderlist]").attr("action", "./orderlist_excel.php");
							$("form[name=frmorderlist]").submit();
							$("form[name=frmorderlist]").attr("action", "#");
						});
					});
					</script>
				</div>
				...
				<div class="tbl_head01 tbl_wrap">
					...
					<tr>
						...
						<!--
						<th scope="col" rowspan="3">플랫폼</th>
						<th scope="col" rowspan="3">콘텐츠제공자</th>
						<th scope="col" rowspan="3">추천인(mother)</th>
						<th scope="col" rowspan="3">결제전(son)</th>
						-->
						
						<th scope="col" rowspan="3">리워드</th>
						<th scope="col" rowspan="3">투표자상세보기</th>
						<th scope="col" rowspan="3">보기</th>
					</tr>
					<tr>
						<th scope="col" id="th_ordnum" colspan="2"><a href="<?php echo title_sort("od_item_names", 1)."&amp;$qstr1"; ?>">강의명</a></th>
					...
					<tr class="orderlist<?php echo ' '.$bg; ?>">
						...
						<?php 
							// 구매 상품에 대한 아이템 퍼센트 요율 가져오기. LHH 2021-02-26
							$cart_info = "";
							$item_info = "";

							$cart_sql = "SELECT * FROM han_shop_cart WHERE od_id = '".$row["od_id"]."' ";
							$cart_info = sql_fetch($cart_sql);

							$item_sql = "SELECT * FROM han_shop_item WHERE it_id = '".$cart_info["it_id"]."'";
							$item_info = sql_fetch($item_sql);
						?>

						<!-- 
						<td rowspan="3" class=""><?php echo $item_info["it_per_platform"];?>%</td>
						<td rowspan="3" class=""><?php echo $item_info["it_per_offerer"];?>%</td>
						<td rowspan="3" class=""><?php echo $item_info["it_per_recommend"];?>%</td>
						<td rowspan="3" class=""><?php echo $item_info["it_per_payer"];?>%</td> 
						-->
						<!-- 
						<td rowspan="3" class=""><?php echo $item_info["it_per_voter"];?>% <button type="button" class="btn btn_03" onclick="javascript:location.href='./vote_list.php?od_id=<?php echo $row['od_id'];?>'">상세보기</button></td>
						-->
						<td rowspan="3" class=""><button type="button" class="btn btn_03" onclick="javascript:location.href='./reward_detail.php?od_id=<?php echo $row['od_id'];?>'">상세보기</button></td>
						<td rowspan="3" class=""><button type="button" class="btn btn_03" onclick="javascript:location.href='./vote_list.php?od_id=<?php echo $row['od_id'];?>'">상세보기</button></td>
					...
					<tr class="<?php echo $bg; ?>">
						<?php
						$item_names = str_replace("|", ", ", $row["od_item_names"]);
						?>
						<td headers="th_ordnum" class="td_odrnum2" colspan="2">
							<strong><?php echo $item_names;?></strong>
						</td>
						<td headers="th_odrid">
							<?php if ($row['mb_id']) { ?>
