...
<!-- 기존 여분필드 input을 아예 빼놔서 hidden 처리해서 폼안에 다시 추가 LHH 2021-02-25 -->
<?php for ($i=1; $i<=10; $i++) { ?>
<input type="hidden" name="it_<?php echo $i ?>_subj" id="it_<?php echo $i ?>_subj" value="<?php echo get_text($it['it_'.$i.'_subj']) ?>" class="frm_input">
<input type="hidden" name="it_<?php echo $i ?>" value="<?php echo get_text($it['it_'.$i]) ?>" id="it_<?php echo $i ?>" class="frm_input">
<?php } ?>
...
<input type="hidden" name="it_good_list" value="<?php echo $it["it_good_list"]; ?>">
...
<?php 
$vimeo_sql = "SELECT * FROM han_write_tutor WHERE wr_id = '".$it["it_1"]."'";
$vimeo_row = sql_fetch($vimeo_sql);


$vimeo_arr = explode("\n", $vimeo_row["wr_link1"]);
for($j=0; $j<1; $j++){
	/*
	$vimeo_arr_ex = explode("||", $vimeo_arr[$j]);
	$vimeo_url = explode("/", $vimeo_arr_ex[1]);
	$vimeo_key = $vimeo_url[3];
	$vimeo = @unserialize(file_get_contents("http://vimeo.com/api/v2/video/".$vimeo_key.".php"));
	if($vimeo[0]["thumbnail_large"]){
		$vimeo_thumb['src'] = $vimeo[0]["thumbnail_large"];
		$vimeo_img = '<img src="'.$vimeo_thumb["src"].'">';
	}else{
		$vimeo_img = "없음";
	}
	*/
...
<section id="anc_sitfrm_skin">
    <h2 class="h2_frm">튜플 게시글 연동</h2>
    <?php echo $pg_anchor; ?>
    <div class="local_desc02 local_desc">
        <p>게재 되어있는 튜플 게시글과 연동 시킵니다.</p>
    </div>
	
    <div class="tbl_frm01 tbl_wrap">
        <table>
        <caption>튜플 게시글</caption>
      
        <tbody>
        <tr>
            <th scope="row"><label for="it_skin">연동</label></th>
            <td>
				<button type="button" class="btn btn_03" onclick="javascript:popupOpen(<?php echo $it_id?>);" >연동하기</button>

            </td>
        </tr>
        <tr>
            <th scope="row"><label for="it_skin">현재 연동된 강의</label></th>
            <td>
				<div style="">
				<a href="<?php echo G5_BBS_URL;?>/board.php?bo_table=tutor&wr_id=<?php echo $vimeo_row["wr_id"];?>" target="_new">
					<strong><?php echo $vimeo_row["wr_subject"];?></strong>
				</a>
				</div>
            </td>
        </tr>
...
function popupOpen(it_id){
	var popUrl = "./pop_vimeo.php?it_id="+it_id;	//팝업창에 출력될 페이지 URL
	var popOption = "width=1024, height=840, resizable=no, scrollbars=no, status=no;";    //팝업창 옵션(optoin)
		window.open(popUrl,"pop_vimeo",popOption);
	}
</script>
					...
					<div class="tbl_frm01 tbl_wrap table-responsive">
						<table style="margin-top:10px">
							...
							<tbody>
								...
								<tr>
									<th scope="row"><label for="it_cust_price">퍼센트 요율</label></th>
									<td>
										<p style="margin:10px">
											<span style="width:100px;display:inline-block;">플랫폼</span>:
											<input type="text" class="frm_input onlynumber it_per" name="it_per_platform" id="it_per_platform" value="<?php echo ($it['it_per_platform'] == "")? $default['de_per_platform']:$it['it_per_platform']; ?>" style="width:100px;" /> %
										</p>
										<p style="margin:10px">
											<span style="width:100px;display:inline-block;">콘텐츠제공자</span>:
											<input type="text" class="frm_input onlynumber it_per" name="it_per_offerer" id="it_per_offerer" value="<?php echo ($it['it_per_offerer'] == "")? $default['de_per_offerer']:$it['it_per_offerer']; ?>" style="width:100px;" /> %
										</p>
										<p style="margin:10px">
											<span style="width:100px;display:inline-block;">추천인(Mother)</span>:
											<input type="text" class="frm_input onlynumber it_per" name="it_per_recommend" id="it_per_recommend" value="<?php echo ($it['it_per_recommend'] == "")? $default['de_per_recommend']:$it['it_per_recommend']; ?>" style="width:100px;" /> %
										</p>
										<p style="margin:10px">
											<span style="width:100px;display:inline-block;">결제자(son)</span>:
											<input type="text" class="frm_input onlynumber it_per" name="it_per_payer" id="it_per_payer" value="<?php echo ($it['it_per_payer'] == "")? $default['de_per_payer']:$it['it_per_payer']; ?>" style="width:100px;" /> %
										</p>

										<?php 
										// 연동된 튜터게시글이 있을 경우 투표자 퍼센트요율도 생성
										?>
										<p style="margin:10px">
											<span style="width:100px;display:inline-block;">투표자</span>:
											<input type="text" class="frm_input onlynumber it_per" name="it_per_voter" id="productDeliveryPrice" value="<?php echo ($it['it_per_voter'] == "")? $default['de_per_voter']:$it['it_per_voter']; ?>" style="width:100px;" /> %
										</p>
										...
										<div class="compare_wrap srel">
											<section class="compare_left">
												<h3>투표리워드ID</h3>
												<label for="sch_relation_2" class="sound_only">상품분류</label>
												<span class="srel_pad">
													<select id="sch_relation_2">
														<option value=''>아이디</option>
													</select>
													<label for="sch_name_2" class="sound_only">상품명</label>
													<input type="text" name="sch_name_2" id="sch_name_2" class="frm_input" size="15">
													<button type="button" id="btn_search_item_2" class="btn_frmline">검색</button>
												</span>
												<div id="relation_2" class="srel_list">
													<p>아이디를 입력해주세요</p>
												</div>
												<script>
												$(function() {
													$("#btn_search_item_2").click(function() {
														var mb_id = $("#sch_name_2").val();
														

														if(mb_id == "") {
															$relation_2.html("<p>아이디를 입력해주세요.</p>");
															return false;
														}

														$("#relation_2").load(
															"./item_get_good_mb.php",
															{ mb_id: mb_id}
														);
													});

													$(document).on("click", "#relation_2 .add_item_2", function() {
														// 이미 등록된 상품인지 체크
														var $li = $(this).closest("li");
														var mb_id = $li.find("input:hidden").val();
														var mb_id2;
														var dup = false;

														
														$("#reg_relation_2 input[name='good_id_list[]']").each(function() {
															mb_id2 = $(this).val();
															if(mb_id == mb_id2) {
																dup = true;
																return false;
														...
														var cont = "<li>"+$li.html().replace("add_item_2", "del_item_2").replace("추가", "삭제").replace("good_id_list", "good_id_list_add")+"</li>";
														var count = $("#reg_relation_2 li").size();

														if(count > 0) {
															$("#reg_relation_2 li:last").after(cont);
														} else {
															$("#reg_relation_2").html("<ul>"+cont+"</ul>");
														}

														$li.remove();
													});

													$(document).on("click", "#reg_relation_2 .del_item_2", function() {
														if(!confirm("아이디를 지우시겠습니까?"))
															return false;
														...
														var count = $("#reg_relation_2 li").size();
														if(count < 1)
															$("#reg_relation_2").html("<p>선택된 아이디가 없습니다.</p>");
													});
												});
												</script>
											</section>
											...
											<section class="compare_right">
												...
												<div id="reg_relation_2" class="srel_sel">
													<?php
													$str = array();
													$good_list_arr = explode(",", $it["it_good_list"]);
													$good_list_arr = array_filter($good_list_arr);
													for($g=0; $g<count($good_list_arr); $g++)
													{
													...
													?>
														<li>
															<input type="hidden" name="good_id_list_add[]" value="<?php echo  $good_list_arr[$g]; ?>">
															<div class="list_item"><?php echo $good_list_arr[$g]; ?></div>
															<div class="list_item_btn"><button type="button" class="del_item_2 btn_frmline">삭제</button></div>
														</li>
													<?php
														//$str[] = $row['it_id'];
													}
													$str = implode(",", $good_list_arr);
...
function fitemformcheck(f)
{
	...
	*/
	...
	// 퍼센트 요율 합계가 100이 아니면 통과 못하게 LHH 2021-03-02
	/*
	var per_sum = 0;
	$(".it_per").each(function(i,e){
		per_sum = per_sum + parseInt($(e).val());
	});

	if(per_sum != 100){
		alert("요율의 합계를 100으로 맞춰주세요");
		console.log(per_sum);
		f.it_per_platform.focus();
		per_sum = 0;
		return false;
	}
	*/
