...
if(!$_POST['mb_id'] && !$_POST['chk_all_mb'] && $_POST['cp_method2'] == 1)
...
if($w == '') {
	...
	if($cp_method2 == '2' && $cp_cnt > 0){
		$cp_rand_id = "";
		for($i = 0; $i < $cp_cnt; $i++){
			$j = 0;
			$rand_id = "";
			do {
				$rand_id = get_coupon_rand_id($cp_string, $cp_int);
				$cp_rand_id .= $rand_id."\n";

				$sql3 = " select count(*) as cnt from g5_shop_coupon_log where cp_id = '".$cp_id."' and rand_id = '".$rand_id."' ";
				$row3 = sql_fetch($sql3);
			...
			$sql2 = " insert g5_shop_coupon_log
						set cp_id = '".$cp_id."'
						,	rand_id = '".$rand_id."'
					";
			sql_query($sql2);
...
// 쿠폰생성알림 발송
if($w == '' && $cp_method2 == '1' && ($_POST['cp_sms_send'] || $_POST['cp_email_send'])) {
