...
// 파일정보
if($w == "u") {
    $sql = " select it_img1, it_img2, it_img3, it_img4, it_img5, it_img6, it_img7, it_img8, it_img9, it_img10, it_zoom_img
...
if ($it_zoom_img_del) {
    $file_img11 = $it_img_dir.'/'.$it_zoom_img;
    @unlink($file_img11);
...
if ($_FILES['it_zoom_img']['name']) {
    if($w == 'u' && $it_zoom_img) {
        $file_img11 = $it_img_dir.'/'.$it_zoom_img;
        @unlink($file_img11);
        delete_item_thumbnail(dirname($file_img11), basename($file_img11));
    }
    $it_zoom_img = it_img_upload($_FILES['it_zoom_img']['tmp_name'], $_FILES['it_zoom_img']['name'], $it_img_dir.'/'.$it_id);
}
...
$it_dc_price = $productDcPrice;
$it_dc_rate = $productDcRate;
$it_dc_flg = $productDcFlg;

$it_stock_qty = "1000000"; // 수량 제한 없음

// 상품에 좋아요 명단 업데이트 LHH 2021-03-02
$it_good_list = @implode(",", $_POST["good_id_list_add"]);
...
$sql_common = " ca_id               = '$ca_id',
                ...
                it_price            = '$it_price',
				it_dc_price         = '$it_dc_price',
				it_dc_rate          = '$it_dc_rate',
				it_dc_flg           = '$it_dc_flg',
                it_point            = '$it_point',
                ...
                it_9                = '$it_9',
                it_10               = '$it_10',
				it_lect_type        = '$it_lect_type',
				it_lect_date        = '$it_lect_date',
				it_lect_day         = '$it_lect_day',
				it_t_name           = '$it_t_name',
				it_score            = '$it_score',
				it_level            = '$it_level',
				it_keyword          = '$it_keyword',
				it_zoom             = '$it_zoom',
				it_zoom_url         = '$it_zoom_url',
				it_zoom_pw          = '$it_zoom_pw',
				it_zoom_img         = '$it_zoom_img',
				it_per_platform		= '$it_per_platform',
				it_per_offerer			= '$it_per_offerer',
				it_per_recommend	= '$it_per_recommend',
				it_per_payer			= '$it_per_payer',
				it_per_voter			= '$it_per_voter',
				it_good_list		= '$it_good_list'
                ";
...
{
	if($w == "u"){
		sql_query("DELETE FROM han_shop_item_list WHERE it_id = '".$it_id."' ");
	}

	// 선택강의 등록
	$vItemIdx = explode(",", $insertItemCode);
	for ($i=0; $i<count($vItemIdx); $i++)
    {
        if (trim($vItemIdx[$i]))
        {
            $sql = " insert into han_shop_item_list
                        set it_id  = '$it_id',
                            it_id2 = '$vItemIdx[$i]',
							ir_order = '$insertItemOrder[$i]',
                            ir_no = '$i' ";
            sql_query($sql, false);
        }
    }
