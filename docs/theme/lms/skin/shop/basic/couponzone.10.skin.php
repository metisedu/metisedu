...
<section class="couponzone_list">
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $print_cp_price = $row['cp_type'] ? '<b>'.$row['cp_price'].'</b> %' : '<b>'.number_format($row['cp_price']).'</b> MTS';
        ...
        if( $row['cp_minimum'] ){   // 쿠폰에 최소주문금액이 있다면
        	$coupon .= '<li>최소주문금액 : <span class="cp_evt"><b>'.number_format($row['cp_minimum']).'</b>MTS</span></li>';
        }
